...
module RakeTasksApp
  class Application < Hanami::Application
    configure do
      ...
      #
      # sessions :cookie, secret: ENV['RAKE_TASKS_APP_SESSIONS_SECRET']
      ...
      #
      layout :application # It will load RakeTasksApp::Views::ApplicationLayout
      ...
      #
      security.content_security_policy "base-uri 'self'; default-src 'none'; script-src 'self'; connect-src 'self'; img-src 'self'; style-src 'self'; font-src 'self'; object-src 'self'; child-src 'self'; media-src 'self'"

      ##
      # FRAMEWORKS
      #

      # Configure the code that will yield each time RakeTasksApp::Action is included
      ...
      # Configure the code that will yield each time RakeTasksApp::View is included
      # This is useful for sharing common functionality
      #
      # See: http://www.rubydoc.info/gems/hanami-view#Configuration
      view.prepare do
        include Hanami::Helpers
        include RakeTasksApp::Assets::Helpers
      end
    end
